...
- [x] Roleta a cada 5 min para capturar um pokemon aleatório.  [completion:: 2025-12-05]
- [x] Atualização automática no front das temperaturas recebidas do back, sem precisar dar refresh(mostrar timer na tela)
- [x] Pesquisar temperatura(Na page de Explorar, junto a api de pokemon) de um lugar e retornar pokemons de um tipo que combine com o clima (Ex. Temperatura > 30 = pokemons de fogo)
- [x] Melhorar as funções de perfil, exibindo pokemon capturados, escolhendo uma local como preferido, para aplicar o monitoramento de tempo.
***
...
### :LiGithub: feat/auth
##### O Cadeado: Autenticação e Segurança (JWT)
Até este ponto, o sistema era uma "casa de portas abertas". Qualquer um podia acessar o Dashboard ou explorar a API. O desafio agora era implementar um sistema de login robusto, seguro e _stateless_ (sem sessão no servidor).

Como proteger rotas no Backend (NestJS) e controlar o acesso no Frontend (React) garantindo que senhas nunca vazem e que o usuário permaneça logado mesmo se atualizar a página?

##### Passport + JWT + Bcrypt
Utilizei a "Santíssima Trindade" da segurança em Node.js.
1. **Bcrypt:** Para criptografar senhas antes de salvar no Mongo. (Senha "123456" vira `$2b$10$EixZa...`).
2. **Passport:** Para gerenciar as estratégias de login.
3. **JWT (JSON Web Token):** O "crachá" digital que o usuário recebe ao logar.

No Backend, configurei duas estratégias distintas. Uma para o momento do login e outra para verificar quem já está logado:


```TypeScript title=auth
...
No Frontend, a mágica acontece com o **Interceptador do Axios**. Em vez de adicionar o token manualmente em cada chamada, configurei o axios para injetar o token automaticamente se ele existir no `localStorage`:

```TypeScript title=frontend/services/api.ts
...
### :LiGithub: feat/final-updates
#### A Roleta Pokémon

Para tornar o sistema mais engajador e diferenciar o projeto de um simples CRUD, decidi criar uma funcionalidade divertida no perfil do usuário: uma roleta para capturar Pokémons.

##### O Desafio
Como limitar o usuário para não "spamar" o botão e capturar mil pokémons de uma vez? Precisava de uma lógica de _Cooldown_ (tempo de recarga) segura, validada no servidor.

No banco de dados do usuário, adicionei o campo `lastSpin` (data da última tentativa). A lógica é matemática simples: `Agora - Último Giro >= 5 minutos?`.


**No Backend (NestJS):** A validação ocorre antes mesmo de sortear o Pokémon. Se o usuário tentar burlar o frontend, o backend barra.


```TypeScript fold=users.service.ts
// 
async spinRoulette(userId: string) {
  ...
  const now = new Date();
  const diffInMinutes = (now.getTime() - user.lastSpin.getTime()) / 60000;

  if (diffInMinutes < 5) {
    throw new HttpException('Ainda em cooldown!', 429);
  }

  // Se passou, sorteia ID de 1 a 1025 e salva na coleção
  // ...
}
```

**No Frontend:** Para melhorar a UX, não deixo o usuário adivinhar. Criei um **Timer Regressivo** no botão que mostra exatamente quanto tempo falta.

#### Radar Climático (Weather Matcher)

Esta foi a cereja do bolo. Já tínhamos dados de Clima e dados de Pokémon. Por que não uni-los? A ideia foi: **"Qual tipo de Pokémon apareceria no clima da minha cidade agora?"**.

### O Desafio

Criar uma lógica que não fosse apenas aleatória, mas que fizesse sentido "biológico" (ex: Pokémons de Água na chuva, Gelo no frio, Fantasma na neblina).

### A Solução: Função Determinística Criativa

Criei uma função utilitária (`weather-matcher.ts`) que recebe os dados do OpenWeather e aplica uma árvore de decisão baseada em prioridades.

### Lógica Aplicada (`weather-matcher.ts`)

A lógica prioriza condições extremas primeiro, depois temperatura e ciclo dia/noite.


```TypeScript title=TypeScript
export function determinePokemonType(weather: any, isNight: boolean): string {
  const condition = weather.weather[0].main.toLowerCase();
  const temp = weather.main.temp;

  // 1. Condições Extremas (Prioridade Máxima)
  if (condition.includes("thunder")) return "electric"; // Raios = Elétrico
  if (condition.includes("snow")) return "ice";         // Neve = Gelo
  if (condition.includes("rain")) return "water";       // Chuva = Água

  // 2. Atmosfera Misteriosa
  if (["mist", "fog"].includes(condition)) return "ghost"; // Neblina = Fantasma

  // 3. Influência da Temperatura
  if (temp >= 30) return "fire";   // Muito quente = Fogo
  if (temp >= 25) return "ground"; // Quente/Seco = Terra
  
  // 4. Ciclo Noturno
  if (isNight) return "dark"; // Se for noite e o tempo estiver normal = Dark

  return "normal"; // Fallback
}
```

No Frontend, adicionei um **Autocomplete de Cidades** que consulta a API de Geocoding para que o usuário não precise adivinhar como se escreve "Tóquio" ou "Nova Iorque".