...
export function PokemonCard({ data }: { data: PokemonDetails }) {
  ...
  return (
    <Card className={cn(
      ...
      <div className="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-white/20 blur-xl transition-all group-hover:bg-white/30" />
          ...
          <div className="flex gap-2 mt-1">
            {data.types.map((t) => (
              <Badge 
                key={t.type.name} 
                variant="secondary" 
                className="bg-white/20 text-white backdrop-blur-sm border-transparent hover:bg-white/30"
              >
                {t.type.name}
              </Badge>
            ))}
